...
void re_paint(int x_left,int y_left,int x_right, int y_right, int x_pos, int y_pos){
    _count++;
    for(int i=x_left;i<=x_right;i++)
        for(int j=y_left;j<=y_right;j++){
            if (i==x_pos && j==y_pos) continue;
            else arr[i][j] = _count;
        }
}

void paint(int x_left,int y_left,int x_right, int y_right, int x_pos, int y_pos){
    ...
    if (x_pos<=x_tmp && y_pos<=y_tmp){// neu x,y o phan 1
        ...
        paint(x_tmp+1,y_tmp+1,x_right,y_right,x_tmp+1,y_tmp+1);//phan 4
    }
...
void solve(){
    ...
    for(int i=1;i<=maxn;i++){
        for(int j=1;j<=maxn;j++) cout << arr[i][j]<<' ';
        cout << '\n';
    }
